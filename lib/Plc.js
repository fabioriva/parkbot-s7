...
class PLC extends EventEmitter {
  ...
  async main (def, obj) {
    try {
      ...
      const buffer = await this.client.ReadArea(area, dbNumber, start, amount, wordLen)
      /* Log */
      const log = new Log(buffer.slice(0, LOG_LEN))
      if (log.etx === 0x03 && log.stx === 0x264) {
        ...
        this.emit('log', log)
        // await this.client.WriteArea(0x84, def.DATA_READ.dbNumber, 0, LOG_LEN, 0x02, Buffer.alloc(LOG_LEN))
      }
      this.client.WriteArea(0x84, def.DATA_READ.dbNumber, 0, LOG_LEN, 0x02, Buffer.alloc(LOG_LEN))
      /* Data */
      await Promise.all([
  ...
  async run (def, obj) {
    try {
      this.online = await this.client.ConnectTo(this.params.ip, this.params.rack, this.params.slot)
